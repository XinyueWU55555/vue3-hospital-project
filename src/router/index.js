...
const routes = [
  {
    path: '/',
    component: () => import('../views/Main.vue'),//动态导入
    name: 'main',
    children: [
      {
        path: 'dashboard',
        ...
        children: [
          {
            path: '',
            alias: ['admin'],
            meta: { id: '1', name: '账号管理', icon: 'Avatar', path: '/auth/admin', describe: '管理员可以进行编辑，权限修改后需要进行登出才会生效' },
      ...
      },
      {
        ...
        children: [
          ...
          {
            path: 'order',
            meta: { id: '2', name: '订单管理', icon: 'List', path: '/vppz/order', descirbe: 'C端下单后可以查看所有订单状态，已支付的订单可以完成陪护状态修改' },
